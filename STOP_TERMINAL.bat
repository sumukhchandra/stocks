...
echo Stopping Streamlit Server (Port 8501)...
...
taskkill /FI "WINDOWTITLE eq NSE_FastAPI_Backend*" /F /T >nul 2>&1
for /f "tokens=5" %%a in ('netstat -aon ^| find ":8000" ^| find "LISTENING"') do taskkill /f /pid %%a >nul 2>&1

echo Stopping Auto-Trader Daemon & Public Tunnels...
taskkill /FI "WINDOWTITLE eq NSE_Auto_Trader*" /F /T >nul 2>&1
taskkill /FI "WINDOWTITLE eq NSE_Localtunnel*" /F /T >nul 2>&1
taskkill /FI "WINDOWTITLE eq NSE_Localtunnel_Backend*" /F /T >nul 2>&1
